...
[docs/GUIDE.md](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md) is the
reference: what is measured, what is not, and why. This page is the short way
in. A form written the way you already write one, then the two or three lines
that put it in the accessibility tree, then how to check that they worked.
...
them are in the tree. See [the claim, stated
precisely](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#the-claim-stated-precisely).
...
- `enable(root)` names every widget that carries its own words and gives every
  widget the right control type. The captions become `TextControl`s that read
  themselves, the buttons become named `ButtonControl`s, and the checkbox
  becomes a `CheckBoxControl` whose `ToggleState` is correct and follows
  `overwrite` with nothing else said. The status line is named from `status` and
  *stays* in step with it: the label told Tk which variable it shows, so
  `enable()` reads that off the widget and follows it. Since 0.6.0 that costs no
  call at all. Set the variable, and what a client reads changes with what is on
  screen.
- `label_for(caption, entry)` says the one thing nothing can read back. An entry
  has no words of its own, and in Tk the label that names it is a *sibling*. No
  part of the toolkit records which widget a caption speaks for, so no amount of
  reading the entry will find it. Said once, the entry answers to `'Task file'`.
  The colon comes off, because every caption in a form has one and none of them
  is part of a control's name.

That is the whole of it for this window. There is no third call to remember.
Anything else you might want to say is in the [API
table](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#the-api), and
most forms need none of it.
...
version](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#the-caption-an-entry-has-and-tk-does-not-record).
...
- **`NAME_NOT_UNIQUE`** is the one to fix. Both `Browse...` buttons are
  ...
  screen reader user hears the same announcement for two controls that do
  different things, and a locator asking for "the Browse... button" gets
  whichever one the tree hands back first. Nothing about either button on its
  own is wrong, which is why it takes a whole-window check to see it. Two fixes,
  and both remove the gap from this report: qualify the captions by hand with
...
guide](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#closing-the-gap-compare-it-with-what-a-client-sees).
...
- No screen reader has been in the room. Everything this project claims is read
  ...
  can read this tree" is evidenced. "NVDA says the right thing at the right
  moment" is not. The paragraph above is what the tree supports, and it is not a
  transcript. [The checklist that would close
  it](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#not-yet-verified-a-real-screen-reader).
...
- [Is this for
  you?](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#is-this-for-you),
  including the two cases where the answer is "use something else".
- [Caveats worth
  knowing](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#caveats-worth-knowing):
  a name goes stale after `config(text=...)`, disabled state is not conveyed,
  and the rest.
- [COVERAGE.md](https://github.com/HuzPro/tk-uia/blob/main/COVERAGE.md): every
  widget class in both toolkits, measured bare, after `enable()`, and after the
  naming a well-behaved application adds.
- [The whole
  API](https://github.com/HuzPro/tk-uia/blob/main/docs/GUIDE.md#the-api):
  seventeen calls, and most forms need three.
