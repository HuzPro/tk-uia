...
Driven entirely by a fake interpreter: a spec that needed a real Tk 9.1 could
not be written today, and one that needed a real Tk at all would not run on the
lane that proves this package is import-clean off Windows.
"""
...
def test_a_tk_that_answers_for_itself_is_left_alone_rather_than_annotated_over() -> (
    ...
    # And it got there without running a single one of those commands. They are
    # still in beta and have never been executed here: deferring to them is a
    # decision, calling them is a guess.
...
def test_a_tk_that_is_not_talking_to_windows_is_left_entirely_alone() -> None:
    ...
    # Then nothing happens, and nothing raises. This answer is what lets a
    # cross-platform application call `enable()` unconditionally rather than
    # wrapping it in a platform check of its own.
