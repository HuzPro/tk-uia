...
- **The README's `describe()` sample was three versions stale, and a count
  ...
  the window's fixed `geometry()`, and the report printed beneath it said
  three: a frame nobody packed, its child, and the notebook tab nobody opened,
  none of which the packer dropped. The packer warning is true and is still
  there, as the thing to fear rather than as what happened here.
